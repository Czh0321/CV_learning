...
# === 迁移学习时需替换：模型导入 ===
from model import SEModule, Bottleneck,MobileNetV3
...
# 初始化参数
    ...
    # === 迁移学习时需替换：权重保存路径 ===
    torch.save(best_model_wts, 'best_MobileNetV3mask_model.pth')
...
if __name__ == '__main__':
    # === 迁移学习时需替换：模型实例化 ===
    model = MobileNetV3(Bottleneck)
    train_dataloader, val_dataloader = train_val_data_process()
    # # === 迁移学习时需替换：训练轮数 + 早停耐心值 ===
    # train_process = train_model_process(model, train_dataloader, val_dataloader, num_epochs=20, patience=5)

    # === 加载最优权重，跑一遍 test ===
    model.load_state_dict(torch.load('best_MobileNetV3mask_model.pth', weights_only=True))
